...
public class tests {
    ...
    static void loadData() throws Exception {
        ...
        input = mapper.readValue(new File(INPUT_PATH), Input.class);
        assertNotNull(input);
        assertFalse(input.graphs.isEmpty(), "Graphs should not be empty");
    }

    @Test
    @Order(1)
    @DisplayName("Full SCC → Condensation → Topo → DAG-SP pipeline")
    void fullPipeline() throws Exception {
        for (var gEntry : input.graphs) {
            ...
            assertEquals(new HashSet<>(gEntry.nodes), all);
            ...
            assertNotNull(shortest.distances);
            assertTrue(shortest.distances.containsKey(source));

            // Longest paths
            Path dagSP2 = new Path(condAdj, new Metrics());
            Path.Result longest = dagSP2.longestPathsFrom(source, topoOrder);
            assertNotNull(longest.distances);
    ...
    @AfterAll
    static void writeOutputs() throws Exception {
        // === Write JSON ===
        ObjectMapper mapper = new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT);
        ...
        System.out.println("Output written to: " + OUTPUT_JSON_PATH + " and " + OUTPUT_CSV_PATH);
    }
}
